...
export const EditorArea: React.FC<EditorAreaProps> = ({
  ...
  const [editingChart, setEditingChart] = useState<NoteChart | undefined>(
    undefined,
  );
    ...
    try {
      ...
      if (response.text) {
        editor?.commands.insertContent(
          `<p><strong>AI Image Analysis:</strong> ${response.text}</p>`,
        );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("image-dropped", handleImageDropped);
  }, [note, deleteImage]);
    ...
    editorProps: {
      handleClick: (view, pos, event) => {
        ...
        if (wikiLinkNode) {
          const targetNoteText =
            wikiLinkNode.getAttribute("data-target") ||
            wikiLinkNode.textContent?.replace(/^\[\[/, "").replace(/\]\]$/, "");
          if (targetNoteText && onNavigateToNote) {
            const targetNoteObj = data.notes.find(
              (n) => n.title.toLowerCase() === targetNoteText.toLowerCase(),
            );
            if (targetNoteObj) {
              onNavigateToNote(
                targetNoteObj.id,
                targetNoteObj.collectionId,
                targetNoteObj.workspaceId,
              );
            } else {
      ...
      handleDrop: (view, event, slice, moved) => {
        if (draggedImageData) {
          ...
          if (coordinates) {
            const imageNode =
              view.state.schema.nodes.imageResize ||
            ...
            if (draggedImageData.id) {
              window.dispatchEvent(
                new CustomEvent("image-dropped", {
                  detail: { id: draggedImageData.id },
                }),
              );
            }
  ...
  const handleSaveAsTemplate = useCallback(async () => {
    ...
    if (name) {
      addTemplate(name, editor.getHTML());
      showToast("✓ Saved as template");
    }
  }, [editor, note, addTemplate, showToast]);
    ...
    (content: string, wordCount: number) => {
      ...
      updateTimeoutRef.current = setTimeout(async () => {
        ...
        try {
          ...
          if (
            !lastVersion ||
            Math.abs(lastVersion.content.length - content.length) > 50
          ) {
            const newVersion = {
  ...
  const handleWikiLink = () => {
    ...
    editor
      ...
      .insertContent({
        type: "wikiLink",
        attrs: { target: selectedText.trim() },
      })
      .insertContent(" ")
      .run();
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden relative">
      ...
      <div className="flex flex-col border-t border-border bg-background shrink-0 no-print safe-area-bottom sticky bottom-0 z-[60] shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] w-full max-w-[100%]">
        ...
        {/* Bottom Row: Actions */}
        <div className="flex items-center justify-between gap-4 overflow-x-auto min-w-0 px-4 py-2">
          <div className="flex items-center gap-2 shrink-0">
            <div className="hidden sm:flex items-center gap-1.5 px-2 py-1 bg-surface-active rounded border border-border text-xs text-text-muted h-8">
              <span className="flex items-center gap-1">
                <span className="text-text-primary text-[10px] w-3 h-3 flex items-center justify-center bg-border rounded">
                  A
                </span>{" "}
          ...
          <div className="flex items-center gap-1.5 shrink-0 no-print">
            ...
            <label
              className="flex items-center justify-center px-2 py-1 text-sm bg-surface-active text-text-primary hover:bg-border rounded-md font-medium transition-colors h-8 w-8 cursor-pointer group"
              title="Import File (.md or .txt)"
            >
              <Upload size={14} />
              <input
                type="file"
            ...
            <div className="relative group/templates">
              ...
              <div className="absolute bottom-full right-0 mb-2 w-56 bg-surface border border-border rounded-md shadow-xl opacity-0 invisible group-hover/templates:opacity-100 group-hover/templates:visible transition-all z-50 py-1 flex flex-col max-h-64 overflow-y-auto transform origin-bottom-right">
                {!data.templates || data.templates.length === 0 ? (
                  ...
                  data.templates.map((t) => (
                    <div
                      key={t.id}
                      className="flex items-center justify-between hover:bg-surface-active group/item px-1"
                    >
                      <button
                        onClick={() => {
                      ...
                      <button
                        ...
                        className="p-1.5 text-text-muted hover:text-red-500 opacity-0 group-hover/item:opacity-100 transition-opacity"
                      >
                        <Trash2 size={12} />
                      </button>
                    </div>
            ...
            <button
              ...
              className={`flex items-center justify-center px-2 py-1 text-sm bg-surface-active hover:bg-border text-text-primary rounded-md font-medium transition-colors h-8 w-8 disabled:opacity-50`}
            ...
            <button
              ...
              className={`flex items-center justify-center w-8 h-8 rounded-md transition-colors ${
                isSpeaking
                  ? "bg-accent/20 text-accent"
                  : "bg-surface-active hover:bg-border text-text-primary"
              }`}
            ...
            {onOpenSettings && (
              <button
                ...
                className="flex items-center justify-center w-8 h-8 bg-surface hover:bg-surface-hover rounded-md text-text-secondary hover:text-text-primary transition-colors shrink-0"
                title="Settings"
              >
                <svg
                  width="18"
